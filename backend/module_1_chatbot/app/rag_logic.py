# app/rag_logic.py (Versi Final dengan State Eskalasi + Intent Classifier)
import os
import requests
import chromadb
...
# --- Inisialisasi Model & DB (Tetap sama) ---
model = None
collection = None 
try:
...
# --- Fungsi Chat History (Tetap sama) ---
...
# --- Fungsi Panggilan AI (Sedikit Modifikasi) ---
...
def call_groq_api(system_prompt: str, user_prompt: str, history: List[Dict]) -> Optional[str]:
    ...
    # Gunakan model lebih ringan untuk klasifikasi cepat jika memungkinkan
...
def get_ai_response(
    ...
    try:
        ...
        if any(s in last_ai_message for s in ["bagaimana?", "tim kami?", "admin?", "teruskan?"]):
            ...
            # [PERUBAHAN] Gunakan AI classifier untuk cek niat user
            ...
            if intent == "AFFIRM":
                ...
                # [PERBAIKAN] Notifikasi admin ganda DIHAPUS dari sini.
                # Admin sudah dinotifikasi saat tawaran pertama kali dibuat.
